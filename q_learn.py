...
def q_learn(env, Q_array, alpha, epsilon, gamma, n_actions):
    total_reward = 0
    t = 0
    state, _ = env.reset()
    terminated = False
    while not terminated:
        ...
        Q_array[state][action] = Q_array[state][action]+ alpha*(reward + gamma*Q_array[s_prime][np.argmax(Q_array[s_prime])] - Q_array[state][action])
        state = s_prime
        total_reward +=  reward
        t += 1
...
for i in range(n_episodes):
    Q_array,_ ,_ = q_learn(env, Q_array, alpha, epsilon, gamma, n_actions)
