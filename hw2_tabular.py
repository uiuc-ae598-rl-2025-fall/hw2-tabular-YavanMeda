...
def evaluate_policy(env, pi_greedy, gamma, n_eval_episodes=32, max_steps=32):
    eval_returns = []
...
def mc_func(env, n_episodes, gamma, epsilon):
    ...
    for i in range(n_episodes):
        ...
        if (i % 100 == 0):
            ...
            avg_returns = evaluate_policy(env, pi_greedy, gamma)
...
def q_learn_func(env, n_episodes, alpha, gamma, epsilon):
    ...
    timestep_q = []
    for i in range(n_episodes):
        eps_k = epsilon(i) if callable(epsilon) else epsilon
...
# for (label, a_func), color in zip(a_list, colors):
#     avg_q, std_q, timestep_q = average_over_func(q_learn_func, 0.1, gamma, 0.5)
#     episodes = timestep_q
#     plot_mean_std(avg_q, std_q, label, color)
